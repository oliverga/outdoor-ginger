...
function Hero({ title, imageSrc, height = "h-96 md:h-[80vh]", bgPos = "object-center", children }) {
  ...
  return (
    <section className="relative mb-6 ">
      <div className={`overflow-hidden grid grid-cols-2 max-w-5xl mx-auto pb-20 md:pb-24 px-8 md:px-0 ${height}`}>
        <div className="place-self-end z-30 w-full flex flex-col justify-end h-fit">
          <h1 className="text-4xl md:text-7xl font-bold text-ogBG-base uppercase m-0">{title}</h1>
          {leftSideChildren}
        </div>
      ...
      <Image src={imageSrc} alt="" width={920} height={500} className={`object-cover ${bgPos} w-full h-full absolute top-0 left-0 z-10`} placeholder="blur" blurDataURL={`{imageSrc}?blur=50`} />
    </section>
  );
}
