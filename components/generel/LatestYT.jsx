...
export default function LatestYT() {
	...
	return (
		<article className="bg-ogPrimary md:px-8 grid grid-cols-2 md:pb-14 grid-rows-latestYT overflow-y-hidden md:overflow-x-hidden  h-fit">
			<h2 className="row-start-1 col-start-1 col-span-2 font-display font-semibold text-ogPrimary-lightest uppercase text-4xl md:text-5xl place-self-center text-center pt-12">
				Latest Adventures
			</h2>
			<div className="py-12 h-full flex md:flex-wrap md:justify-evenly gap-x-8 md:gap-x-0 gap-y-8 self-end row-start-2 col-start-1 col-span-2  md:m-auto z-20 overflow-x-scroll md:overflow-x-visible md:max-w-5xl">
				{videos.length > 0 &&
					videos.map((video) => (
						<div key={video.id} className="aspect-video h-48 md:h-64">
							{/* <iframe
								key={video.id}
